  ...
  const handleImageFiles = (files: FileList) => {
    ...
    const newFiles = Array.from(files).filter(file => {
      ...
      if (file.size > MAX_SIZE_BYTES) {
        alert(`Image "${file.name}" is too large (${(file.size / 1024 / 1024).toFixed(2)}MB). The maximum size is ${MAX_SIZE_MB}MB.`);
        return false;
      }
            ...
            {images.length > 0 ? (
              ...
              >
                <label htmlFor="image-upload" className="flex flex-col items-center justify-center w-full h-full text-center cursor-pointer p-4">
                  ...
                  <p className="text-xs text-text-light-secondary dark:text-text-dark-secondary">PNG, JPG, WEBP (max 4MB each)</p>
                </label>
              </div>
            )}
